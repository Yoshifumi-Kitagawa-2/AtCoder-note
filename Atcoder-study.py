...
##勾配降下法:山を下っていくようにして関数f(x)の最小値を求めるアルゴリズム
#手順
#1.定数αと初期値Xを求める
#2.X = X-α✖︎f'(x)　を繰り返す
#応用例①：合計距離最小化
...
##A*
#A*アルゴリズム
#グラフの最短経路を求める問題にて、計算回数を減らせる問題の一つ
#アイデア
#たとえ始点からの距離が小さくても、明らかに終点から遠そうな頂点は調べない
#そこで、始点から頂点vまでの距離をf(v)、頂点vから終点までの距離の予測値をg(v)とするとき、f(v)+g(v)の小さい頂点から順に調べていく
#なお、g(v)はヒューリスティックコストと呼ばれる
